...
def reorder_matrix_to_global_order(nodes, matrix):
    original_dofs = [dof for node in nodes for dof in (node * 3 - 2, node * 3 - 1, node * 3)]
    sorted_nodes = sorted(nodes)
    sorted_dofs = [dof for node in sorted_nodes for dof in (node * 3 - 2, node * 3 - 1, node * 3)]
...
def read_stiffness_matrices(filepath: str):
    ...
    while i < len(lines):
        ...
        if line.startswith("** ELEMENT NUMBER"):
            ...
            dof_list = [node * 3 - 2 for node in nodes] + [node * 3 - 1 for node in nodes] + [node * 3 for node in nodes]
            n = len(dof_list)
...
if __name__ == "__main__":
    ...
    cross_out_lines_1_based = [21,24,26,27,48,51,54,75,78,79,80,81]  # (1-based)
